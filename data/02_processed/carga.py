...
for c in cycles_config:
    ...
    try:
        ...
        if found_target:
            df_mcq = df_mcq_raw[["SEQN", found_target[0]]].rename(
                columns={found_target[0]: "TARGET"}
            )
        else:
        ...
        if "PAQ650" in df_paq_raw.columns:
            df_paq = df_paq_raw[["SEQN", "PAQ650"]].rename(
                columns={"PAQ650": "Actividad_Fisica"}
            )
        elif "PAQ505" in df_paq_raw.columns:
            df_paq = df_paq_raw[["SEQN", "PAQ505"]].rename(
                columns={"PAQ505": "Actividad_Fisica"}
            )
        else:
        ...
        # --- UNIÓN ---
        df_cycle = (
            df_demo.merge(df_mcq, on="SEQN", how="inner")
