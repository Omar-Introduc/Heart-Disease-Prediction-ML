...
class DecisionTree:
    ...
    def _build_tree(self, X, g, h, depth):
        ...
        if (
            best_split is None or best_split["gain"] < 0
        ):  # Should be gain <= gamma? The formula says Gain - gamma. If result is negative, don't split.
            return {"leaf": True, "weight": weight}

        # Recursively build left and right subtrees
        left_tree = self._build_tree(
            best_split["X_left"], best_split["g_left"], best_split["h_left"], depth + 1
        )
